...
class OfficeModelDbTestCase(BaseTestCase):
    def test_create_office_successful(self):
        ...
        office_name = self.office.create_office({"type": "Transport", "name": "Permernent Secretary"})
    ...
    def test_create_office_duplicate(self):
        ...
        integrity_error = self.office.create_office({"type": "Transport", "name": "Permernent Secretary"})
        self.assertEqual(integrity_error, 'Office Exists')

    def test_create_office_invalid_data(self):
        """Tests data given is invalid"""
        integrity_error = self.office_invalid.create_office({"type": "Transport", "name": ""})
        self.assertEqual(integrity_error, 'Invalid Data')

    def test_get_offices(self):
        """Tests gets all data """
        OfficesModelDb().create_office({"name": "Governor", "type": "Government"})
        OfficesModelDb().create_office({"name": "Senetor", "type": "Government"})
        OfficesModelDb().create_office({"name": "County Rep", "type": "Private Sec"})
    ...
    def test_edit_office_cant_edit_existing_data(self):
        ...
        OfficesModelDb().create_office({"name": "Governor", "type": "Government"})
        OfficesModelDb().create_office({"name": "Assembly Speaker", "type": "Government"})
