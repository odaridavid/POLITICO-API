    ...
    if {'party', 'candidate'} <= set(register_data) and 'Invalid' not in ids:
        ...
        if isinstance(candidate_info, list):
            return make_response(
                jsonify({"status": 201, "data": [{"office": candidate_info[0][0], "user": candidate_info[0][2]}]}), 201)
        elif 'Candidate Conflict' in candidate_info:
            return make_response(jsonify({"status": 409, "error": "Candidate Already Registered"}), 409)
